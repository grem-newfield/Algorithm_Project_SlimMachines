...
inline void def_routes(crow::SimpleApp &app) {
    ...
    CROW_ROUTE(app, "/start_session")([](const crow::request& req) {
        ...
        if (user_id.empty()) {
            ...
            if (validate_user_id(user_id)) {   // Function to validate the user ID from the database
...
// Helper functions to generate a new user ID, validate it, and insert a new user in the database
