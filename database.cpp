...
int insert_or_update_user_data(const std::string &cookie_uuid,
                               const std::string &data_json,
                               const std::string &expiration_date) {
  sqlite3_stmt *stmt;
  const char *sql_check =
      "SELECT COUNT(*) FROM USER_DATA WHERE COOKIE_UUID = ?;";
  const char *sql_insert = "INSERT INTO USER_DATA (COOKIE_UUID, DATA, "
                           "EXPIRATION_DATE) VALUES (?, ?, ?);";
  const char *sql_update = "UPDATE USER_DATA SET DATA = ?, EXPIRATION_DATE = ? "
                           "WHERE COOKIE_UUID = ?;";
...
void delete_expired_cookies() {
  const char *sql =
      "DELETE FROM USER_DATA WHERE EXPIRATION_DATE < DATE('now');";
